...
var mediaTitle = t('application.inlineEditor.media.title');
var mediaEdit = t('application.inlineEditor.media.edit');
...
(function() {
	...
	tinymce.create('tinymce.plugins.MediaPlugin', {
		init : function(ed, url) {
			...
			// Register buttons
			ed.addButton('media', {title : mediaTitle, cmd : 'mceMedia'});
		...
		dataToImg : function(data, force_absolute) {
			...
			img.width = data.width = normalizeSize(data.width || (data.type == 'audio' ? "300" : "378"));
			img.height = data.height = normalizeSize(data.height || (data.type == 'audio' ? "32" : "310"));
        ...
        data['class'] = iframe.attr('class');
			...
			img.attr({
				...
				style : style,
				width : width || (node.name == 'audio' ? "300" : "378"),
				height : height || (node.name == 'audio' ? "32" : "310"),
				hspace : hspace,
				vspace : vspace,
				align : align,
				bgcolor : bgcolor,
